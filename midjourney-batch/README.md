...
> **구글 로그인이 막히거나 로그인 팝업이 무한 로딩될 때 (구글 계정 전용 해결책)**
> 구글은 자동화 브라우저에서의 로그인을 강하게 차단합니다. 이럴 땐 **새로 로그인하지
> 말고, 이미 미드저니에 로그인된 내 크롬을 그대로 빌려** 쓰면 됩니다(재로그인 불필요):
>
> 1. 평소 쓰는 **크롬을 완전히 종료**합니다(작업관리자에서 `chrome.exe` 전부 종료).
> 2. `--use-my-chrome` 를 붙여 실행합니다:
>    ```bash
>    python mj_batch.py --login --use-my-chrome                 # 로그인 상태 확인용
>    python mj_batch.py --prompts <파일>.txt --use-my-chrome    # 실제 실행(매번 이 옵션 필요)
>    ```
> - 이 모드에서는 **실행 중 평소 크롬을 쓸 수 없습니다**(프로필을 점유). 배치가 끝나면
>   다시 자유롭게 쓸 수 있습니다.
> - 디스코드 계정이 있다면, 로그인 창의 **Discord 로그인**도 안정적인 대안입니다.
...
(`--ar` 종횡비, `--raw`, `--no text,...` 네거티브 · V8.2 기준)으로 변환된 `*-midjourney.txt`가
...
- 버전은 현재 기본 **`--v 8.2`** (V8부터 `--style raw` → `--raw`). 고정하려면 각 줄 끝에 `--v 8.2` 추가.
...
- **한 줄 = 프롬프트 하나**
- 빈 줄, `#` 주석 줄은 무시
- `--ar 3:2 --v 8.2` 같은 Midjourney 파라미터도 그대로 붙여쓰면 됨
