...
/* 음식 단독 주제 + 탑뷰 전달 규칙 — 어도비는 크롭을 구매자 몫으로 두므로, 전달 이미지에는 음식과 모든
   접시·그릇·트레이·뚜껑·포장 용기가 가장자리 짤림 0으로 온전히 들어오고 전 영역 탁샤프(블러·얕은심도 없음).
   긍정문으로 서술(미드저니 부정어 유인 방지) — '깊은 심도·모서리까지 초점'으로 블러 불가를 표현 */
const FOOD_TOPVIEW_FULLFRAME = "the food is the sole subject and must be shown COMPLETE and uncropped: the entire dish together with every plate, bowl, tray, lid, wrapper and packaging container is kept whole and fully inside the frame, with a comfortable safe margin of clean space on all four sides so the subject stays well clear of every edge (Adobe buyers do their own cropping, so the delivered shot must contain the whole food and its full packaging intact); the whole subject is rendered tack-sharp and fully in focus from corner to corner with deep front-to-back depth of field, every part of the food and its container crisply detailed and evenly sharp";
...
function buildSlotPrompt(slot, mode, tone = "realism", people = "auto", koreanCast = false, hands = "auto") {
  ...
  return [
    ...
    /* 음식 단독 탑뷰 — 포장 용기까지 짤림 0 + 전 영역 탁샤프(블러 불가) 명시 */
    isFoodTopview ? FOOD_TOPVIEW_FULLFRAME : "",
...
function toMidjourney(slot, mode, tone, people, aspect, hands = "auto") {
  ...
  /* 음식 단독 탑뷰 — 포장 용기 짤림·블러를 --no로 명시 차단 (어도비 전달본은 크롭 없이 전 영역 선명). jpkr는 소프트-소품 공식이라 제외 */
  const foodTopviewMJ = !isPIW && FOOD_RE.test(themeText) && !JPKR_CAFE_RE.test(themeText) &&
...
function buildMidjourneyPrompt(slot, { aspect = "16:9", tone = "realism", people = "none" } = {}) {
  ...
  params.push(`--no text, letters, numbers, logo, watermark, signature, korean text, hangul, calligraphy, wall lettering, framed verse, hanging banner, book cover text, split screen, diptych, collage, grid layout${banPeople ? ", people" : ""}`);
