...
const MIRI_LIFESTYLE_RE = /부동산|아파트|인테리어|모델하우스|임장|매매|이사|입주|열쇠|전세|월세|realestate|real\s*estate|apartment|interior|home|저널|일기|다이어리|독서|공부|힐링|명상|재활용|분리수거|보드게임|가족|일상|라이프스타일|스마트폰|모닝|출근|커피\s*한잔|햇살|창가/i;
...
function toMidjourney(slot, mode, tone, people, aspect, hands = "auto") {
  ...
  /* 골프 — 하늘·잔디·배경 블러 금지 (딥DOF 풍경 선명) */
  if (isGolfMJ) neg += ", blur, blurred background, bokeh, shallow depth of field, out of focus, soft background, hazy sky";
...
function buildMidjourneyPrompt(slot, { aspect = "16:9", tone = "realism", people = "none" } = {}) {
  ...
  if (isGolfBatch) neg += ", blur, blurred background, bokeh, shallow depth of field, out of focus, soft background, hazy sky";
  const params = [];
