      ...
      for (let attempt = 0; attempt < 3 && !cancelRef.current && !timeUp(); attempt++) {
        try {
          const r = await askBrain(
            detailSystem,
            `Topic: "${topic}". Mode: ${mode}.${packageMode ? " PACKAGE MODE ON — this pair shares the SET-WIDE aesthetic register (same palette family, same soft daylight, same setting style, casual home wardrobe register) but different actors and different exact outfits are FINE. Do NOT push differentiation on palette/lighting/mood." : ""} Whole-set overview (${packageMode ? "keep THIS pair visually cohesive with the whole set — only the action/prop varies" : "make THIS pair's props/palette clearly distinct from all"}): ${setSummary}
