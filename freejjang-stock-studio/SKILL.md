...
> 정확한 이유: 어도비가 "어두워서" 거절하는 게 아니라, 어두운·글로우·블러 컷은 실제 거절 사유(노이즈·소프트포커스·노출·아티팩트)
> 발생률이 높아 통과율이 낮다. 그래서 이 유형은 미캔 전용으로 라우팅해 계정 승인율을 보호한다(리스크 기반 기본값).

- **어도비 결함 발생률이 높은 유형** = ①어두운 곳의 과도한 네온·발광 ②소프트/크리미 블러·보케 ③다크·야간·컨셉 렌더. `DARK_GLOW_RE` + `mode(wallpaper/emotional)` + `tone(concept/cinematic)`로 감지. ※ 선명·저노이즈로 검증된 다크 풍경컷은 어도비 승인 가능 → 필요시 수동 승격.
...
### 📈 시그널 루프 (양산 캐논 · `signals/`)

> 감이 아니라 판매 신호로 다음 배치를 정한다. 상세 규칙은 **`/signals/README.md`**.

- **시안 게이트** — 새 주제는 1~3장 시안 → 승인 → 20컷 내외 앵글 분산. 처음부터 대량 생산 금지.
- **형태 4종 라벨** — 배경형 / 제품형 / 인물형 / 설명형. 모든 컷·프롬프트에 부여.
- **컷 단위 성과 기록** — `signals/signals.csv`에 승인 여부·거절사유·DL30·DL90 기록, 90일 데이터로 다음 배치 재투입.
